...
| 9 — Firmware 27.0.6 recovery (preset+JSON catalog) | ✅ done (live verification pending Watchtower) | 2026-05-18 | Speaker auto-upgraded from 27.0.3 → 27.0.6 during a power-cycle. New firmware removes direct-URL `LOCAL_INTERNET_RADIO` from `/sources` entirely; `POST /select` returns 1005 for every previously-working source. **Workaround:** speaker now plays internet radio only via stored presets whose `location` is a JSON catalog URL. New `GET /api/stations/{id}/station.json` endpoint in our app serves `{audio:{streamUrl,...}, name, streamType:"liveRadio"}`. `StationService.play` switched to `storePreset(scratchSlot) + key PRESET_N`. New `POST /api/presets/{slot}/{station}` pins to slots 1..5 so the **Bose IR remote works** again for those slots. `ContentItem` gained `type="stationurl"` attribute. Plain HTTP only — firmware doesn't follow HTTPS. 49 unit tests pass. |
| 8 — Packaging & deployment | ✅ done (software only — Pi deployment runs after first push to GitHub) | 2026-05-17 | Multi-stage `Dockerfile` (Temurin 21 JDK → JRE, fat JAR via `bootJar`, tini PID-1, `HEALTHCHECK` curl on `/api/health`). `.github/workflows/ci.yml` runs `./gradlew build` on PR + push; `.github/workflows/deploy.yml` builds `linux/arm64` and pushes to `ghcr.io/pospon/soundtouch-radio:latest` + `:{sha}` on push to `main`. `application-pi.yml` profile enables buttons + display. `deploy/docker-compose.snippet.yml` is the block to paste into [pospon/homeserver](https://github.com/pospon/homeserver) — Watchtower auto-updates. Plan deviation: **no systemd**; mirrors the `pospon/tuya-horakova` Docker/GHCR/Watchtower pattern instead, which the Pi already runs. Locally built `linux/arm64` image is 366 MB; container boots, serves `/api/stations`, exercises WS-reconnect backoff (verified on Mac via OrbStack). **Pi deployment not yet executed** — push to GitHub + paste compose snippet + reboot Pi is the user-side step that closes the exit criterion. |
...
Added:
- `Dockerfile` — two-stage build. Stage 1: `eclipse-temurin:21-jdk` runs `./gradlew bootJar -x test`. Stage 2: `eclipse-temurin:21-jre` + `tini` + `curl` (for `HEALTHCHECK`) + the fat JAR. Entry is `tini -- java -jar /app/app.jar` so PID-1 signal handling is sane.
- `.dockerignore` — excludes `build/`, `.gradle/`, `.git/`, IDE files, docs.
- `src/main/resources/application-pi.yml` — minimal profile that flips `radio.buttons.enabled` and `radio.display.enabled` to `true`. Activated via `SPRING_PROFILES_ACTIVE=pi` in the docker-compose env.
- `.github/workflows/ci.yml` — runs `./gradlew build` on PRs + push to main. No Docker push; this is the unit-test gate.
- `.github/workflows/deploy.yml` — mirrors `pospon/tuya-horakova`: QEMU + buildx + push `linux/arm64` to `ghcr.io/pospon/soundtouch-radio:latest` and `:{sha}` on push to main. Caches via `type=gha`.
- `deploy/docker-compose.snippet.yml` — the block to paste into the homeserver repo's `docker-compose.yml`. Pulls from GHCR, env `SPRING_PROFILES_ACTIVE=pi` + `SOUNDTOUCH_HOST=${SOUNDTOUCH_HOST}` + `TZ=Europe/Prague`. Passes through `/dev/gpiochip0` + `/dev/i2c-1` and `group_add: [gpio, i2c]`. Traefik labels: `Host(radio.${DOMAIN})` + `tls.certresolver=letsencrypt` + `middlewares=lan-only` (LAN-only access — no API auth, can't be on the public internet). Loadbalancer port 8080.
- `README.md` — top-level repo onboarding, replaces having `docs/README.md` serve double duty.

Plan deviations:
- **No systemd unit.** Plan §8 describes a hand-rolled systemd `.service` + `install.sh`. Replaced with Docker + GHCR + Watchtower auto-update, matching the user's existing homeserver pattern. Pros: zero manual deploys after the first compose-up; cons: container needs explicit GPIO/I²C device passthrough (handled in the compose snippet).
- **No `config/*.yaml` external file.** Plan §8 had `install.sh` copying YAML to `/etc/soundtouch-radio/`. We bake stations + button bindings into the JAR (`application.yml`) and only externalize via env (`SOUNDTOUCH_HOST`). Cleaner for the immutable-container model. If you ever want config without rebuilding, mount a host path with `application-local.yml` and `--spring.config.additional-location=`.
- **Single `latest` tag + commit-SHA tag.** No semantic versioning. Watchtower wants `:latest`; the SHA tag is for traceability.

Verification:
- `./gradlew build` green; 56 unit tests pass. ✅
- `docker buildx build --platform linux/arm64 -t soundtouch-radio:test --load .` succeeds locally on macOS via OrbStack. Image: 366 MB. ✅
- `docker run -p 18080:8080 -e SOUNDTOUCH_HOST=10.0.0.148 soundtouch-radio:test` boots, serves `/api/stations` (200) and `/api/now-playing` (200), `/api/health` returns 503 DOWN (expected — Mac→Docker bridge can't route to the LAN speaker). WS reconnect schedule fires through 1s → 2s backoff confirming the listener works. ✅

Open follow-ups (the actual Phase 8 exit criterion):
- Create `pospon/soundtouch-radio` on GitHub and push. First push triggers `deploy.yml` and lands the image on GHCR.
- Paste `deploy/docker-compose.snippet.yml` into `pospon/homeserver`'s `docker-compose.yml` and add `SOUNDTOUCH_HOST=10.0.0.148` to `/opt/homeserver/.env`. `docker compose up -d soundtouch-radio` once on the Pi.
- Reboot the Pi → systemd brings docker up → docker brings the container up → Vltava-button-press plays Vltava. That's the plan's exit criterion: *"reboot the Pi, system comes back up, station plays via physical button without intervention."*
- Once Phase 6 (button) and Phase 7 (OLED) hardware is wired, that's also satisfied.
